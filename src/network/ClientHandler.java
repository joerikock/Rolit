...
public class ClientHandler extends Thread {
	...
	/**
	 * Geeft als er nog geen spel gestart is het gewenste aantal spelers
	 * terug en anders het aantal spelers die meedoen aan dit spel
	 */
	 ...
	 * @param	game spel waar deze client aan mee gaat doen 
	 */
	...
	/**
	 * Stel de groep van clients in waarmee deze client samen
	 * een Game gaat spelen
	 * @param	party lijst van clients voor hetzelfde spel
	 */
	 ...
	 * @param	lineIn regel van de invoer
	 */
	private void readCommand(String lineIn) {
		try {
			...
			server.addMessage("[!*Oeps, er ging iets fout: " + e.getMessage().toString() + "*]");
		}
	}
     ...
     * @param arg Het met het commando meegestuurde argument
     */
		...
		if (game.getBoard().isTile(move) &&
			...
			if (game.getBoard().isFull()) {
				...
				for (int i = 0; i < partySize; i++) {
					punten[i] = new Integer(game.getBoard().countTiles(game.getPlayers().get(i).getColor())).toString();
					result += game.getPlayers().get(i).getName() + ": " + punten[i] + "\n";
				}
